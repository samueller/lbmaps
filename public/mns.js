const width = height = 575
    ...
    , initialModel = boundsOf => bounds => obsData => hoverPopup => px => pycx => pycxp => {
        const pxp = 1 - px
            , pxy = px * pycx
            , pxyp = px * (1 - pycx)
            , pxpy = pxp * pycxp
            , pxpyp = pxp * (1 - pycxp)
            , py = pxy + pxpy
        return { boundsOf: 1, bounds, obsData, hoverPopup, px, pycx, pycxp, pxp, pxy, pxyp, pxpy, pxpyp, py }
    }
    ...
    , boundsFromUrl = () => new URL(window.location).searchParams.get('bounds') == 'upper'
        ? 1
        : 0
    , model = initialModel(boundsOfFromUrl())(boundsFromUrl())(obsDataCheckbox.checked)(hoverPopupCheckbox.checked)(+pxSlider.value)(+pycxSlider.value)(+pycxpSlider.value)
    ...
    , contours = ({ obsData, boundsOf, bounds, pxy, pxyp, pxpy, pxpyp, py }) => d =>
        svgStringFromCoords(
            obsData
                ? boundsOf == 0
                    ? bounds == 0
    ...
    , drawContours = svg => {
        svg.selectAll('polygon.contour')
            ...
            .join('polygon')
            .attr('class', 'contour')
            .attr('fill', d => d3.schemeSpectral[numContours][numContours - d - 1])
    ...
    , contourTextTransform = model => d =>
        model.obsData || model.bounds == 1
            ? model.boundsOf == 0
                ? model.bounds == 0
                    ? `translate(${[scaleX(0.01), scaleY(model.py - (d+0.5)/numContours)]}) rotate(0)`
                    : `translate(${[scaleX((d+0.5)/numContours), scaleY(0.01)]}) rotate(-90)`
                : model.bounds == 0
                    ? `translate(${[scaleX(model.py - (d+0.5)/numContours), scaleY(0.01)]}) rotate(-90)`
                    : `translate(${[scaleX(0.01), scaleY((d+0.5)/numContours)]}) rotate(0)`
            : model.boundsOf == 0
                ? `translate(${[scaleX((d+0.575)/numContours), scaleY(0.01)]}) rotate(-45)`
                : `translate(${[scaleX(0.01), scaleY((d+0.575)/numContours)]}) rotate(-45)`
, updatePlot = svg => model => {
        if (model.obsData) {
            ...
            svg.select('line.possible-label')
                .attr('x1', scaleX(possibilityWindow[5][0]*7/8 + possibilityWindow[6][0]*1/8))
                .attr('y1', scaleY(possibilityWindow[6][1] - 0.075))
                .attr('x2', scaleX(possibilityWindow[5][0]*3/4 + possibilityWindow[6][0]/4))
                .attr('y2', scaleY(possibilityWindow[6][1]))
                .transition()
                .duration(1000)
                .attr('opacity', 1)
            svg.select('text.possible-label')
                .attr('transform', `translate(${[scaleX(possibilityWindow[5][0]*7/8 + possibilityWindow[6][0]*1/8), scaleY(possibilityWindow[6][1] - 0.075)]})`)
                .transition()
                .duration(1000)
                .attr('opacity', 1)
            svg.select('line.mono-label')
                .attr('x1', scaleX(possibilityWindow[5][0]*0/32 + possibilityWindow[6][0]*32/32 - 0.01))
                .attr('y1', scaleY(possibilityWindow[6][1] - 0.05))
                .attr('x2', scaleX(possibilityWindow[5][0]*0/32 + possibilityWindow[6][0]*32/32 - 0.01))
                .attr('y2', scaleY(possibilityWindow[6][1]))
                .transition()
                .duration(1000)
                .attr('opacity', 1)
            svg.select('text.mono-label')
                .attr('transform', `translate(${[scaleX(possibilityWindow[5][0]*1/8 + possibilityWindow[6][0]*7/8), scaleY(possibilityWindow[6][1] - 0.05)]})`)
                .transition()
                .duration(1000)
                .attr('opacity', 1)
                .text(model.bounds == 0 ? 'Necessary region' : 'Sufficient region')
        } else {
            svg.select('polygon.impossible')
                .transition()
                .duration(1000)
            ...
            svg.select('text.mono-label')
                .transition()
                .duration(1000)
                .attr('transform', `translate(${[scaleX(5/8), scaleY(1/4)]})`)
                .attr('opacity', 1 - model.bounds)
                .text(model.bounds == 0 ? 'Necessary region' : 'Sufficient region')
        }
        svg.selectAll('polygon.contour')
            .join('polygon')
    ...
    , move = () => {
        ...
        if (pyx >= 0 && pyx <= 1 && pyxp >= 0 && pyxp <= 1) {
            ...
            renderMath(document.getElementById('stats-bounds'))(`${round2(lowerBound_)} \\leqslant P(y_x ${model.boundsOf == 0 ? '>' : '<'} y_{x'}) \\leqslant ${round2(upperBound_)}`)
    ...
    , createPlot = svgElement => {
        ...
        hoverPopupCheckbox.addEventListener('input', e =>
